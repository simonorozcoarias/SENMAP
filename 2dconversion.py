...
def conversion2d(file, maxlength, numSeqs, method):
	...
	for te in SeqIO.parse(file, "fasta"):
		...
		if str(te.id).upper().find("_NEGATIVA") != -1:
			order = 0
		elif str(te.id).upper().find("_POSITIVA") != -1:
			order = 1
...
#		# 	order = 24
	...
	save(file+'_'+methodName+'.npy', rep2d.astype(bool))
	print("done!!")
	print("saving labels file...")
	save(file+'_'+methodName+'_labels.npy', labels.astype(bool))
	print("done!!")
...
if __name__ == '__main__':
	seqfile = "/shared/home/rguyot/coffea_genomes/Simon/benchmarking/PC/DL/2_labels_DB.fa"
	method = 4
	filter(seqfile)
